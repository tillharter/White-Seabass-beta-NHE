...
setwd("select path/WSB BOBs runs")

#show working directory
print(getwd())

#list all files
myfiles <- list.files(path="select path/working directory",
...
#exclude the in vivo stress protocol
myfiles<-subset(myfiles, str_detect(myfiles, "01272021")==FALSE)

#exclude the CA treatment
myfiles<-subset(myfiles, str_detect(myfiles, "CA1")==FALSE)
...
#relevel drug
sum_normoxia$drug<-factor(sum_normoxia$drug, levels=c("DMSO", "ISO10", "Am1000"),
                 labels=c("DMSO", "ISO", "ISO+Am"))
sum_hypoxia$drug<-factor(sum_hypoxia$drug, levels=c("DMSO", "ISO10", "Am1000"),
                          labels=c("DMSO", "ISO", "ISO+Am"))
...
normox_halfsat<-data.frame(drug=c("DMSO","ISO", "Am"), 
...
hypox_halfsat<-data.frame(drug=c("DMSO","ISO", "Am"), 
...
F3<-ggplot(data=sum_normoxia, aes(x=CO2, y=sat, fill=drug, colour=drug))+
  ...
  geom_jitter(data=sum_hypoxia, position=position_jitterdodge(jitter.width=0.1, dodge.width=0.2),
              size=1.5, alpha=0.8, shape=21, fill="white", show.legend = FALSE)+
  stat_summary(fun=mean, geom="point", shape=21, size=3, show.legend = T)+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.1, size=0.7, show.legend = T)+
  
  stat_summary(data=sum_hypoxia, fun.data=mean_se, geom="errorbar", width=0.1, size=0.7,
               show.legend = T)+
...
ggsave(filename="Figure 3_WSB bNHE_Sat_02042021.tiff", unit="cm", width=12, height=12, plot=F3)
ggsave(filename="Figure 3_WSB bNHE_Sat_02042021.pdf", unit="cm", width=12, height=12, plot=F3, useDingbats=FALSE)


rm(modsum_hypox_DMSO, modsum_hypox_ISO, modsum_hypox_Am, 
   modsum_normox_DMSO, modsum_normox_ISO, modsum_normox_Am, 
   sum_hypoxia, sum_normoxia, hypox_Am, hypox_DMSO, hypox_ISO,
   normox_Am, normox_DMSO, normox_ISO, F3, xlab, ylab)
...
for (i in 1:(dim(model_dat)[1]/6)){
  ...
  hill <- nlsLM(sat ~ (a*CO2^b)/(c^b+CO2^b), 
                data=data, 
                start = list(a=-50, b=2, c=1), trace=TRUE, 
                control=nls.control(maxiter=1024))
...
model_params$drug<-factor(model_params$drug, levels=c("DMSO", "ISO10", "Am1000"),
                             labels=c("DMSO", "ISO", "ISO+Am"))
str(model_params)
...
F4A<-ggplot(data=subset(model_params, O2=="normoxia"), aes(x=drug, y=c, fill=drug, colour=drug))+
  scale_fill_manual(name="Drug", limits = c("DMSO", "ISO", "ISO+Am"),
                    values = c(col_pal[3], col_pal[4], col_pal[5]))+
  scale_colour_manual(name="Drug", limits = c("DMSO", "ISO", "ISO+Am"),
                      values = c(col_pal[3], col_pal[4], col_pal[5]))+
  scale_x_discrete(labels=c("DMSO", "ISO", "ISO+Am"))+
  ...
  geom_text(label=pval_drug, x=3.5 , y=1.5, colour="black", size=3,
             show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
  geom_text(label=pval_O2, x=3.5 , y=1.4, colour="black", size=3,
             show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
  geom_text(label=pval_inter, x=3.5 , y=1.3, colour="black", size=3,
             show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
  theme_bw()+
  theme(panel.grid.major = element_blank(),
        panel.grid.minor = element_blank(),
...
F4B<-ggplot(data=subset(model_params, O2=="normoxia"), aes(x=drug, y=a, fill=drug, colour=drug))+
  ...
  geom_text(label=pval_drug_vmax, x=3.5 , y=0, colour="black", size=3,
            show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
  geom_text(label=pval_O2_vmax, x=3.5 , y=-6, colour="black", size=3,
            show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
  geom_text(label=pval_inter_vmax, x=3.5 , y=-12, colour="black", size=3,
            show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
...
rm(model_params, pval_drug_vmax, pval_inter_vmax, pval_O2_vmax, ylab)
...
g_comb<-ggdraw() +
  draw_plot(F4A, x = 0.02, y =  1/2, width = (1-0.02), height = 1/2)+
  draw_plot(F4B, x = -0.01, y =  0, width = (1+0.01), height = 1/2)+
...
F5A<-ggplot(data=subset(delta_sat, O2=="normoxia"), aes(x=CO2, y=rel_sat, colour=drug, fill=drug))+
  ...
  geom_jitter(position=position_jitterdodge(jitter.width=0.1, dodge.width=0.2),
              size=1.5, alpha=0.7, shape=21, show.legend = FALSE)+
  stat_summary(fun=mean, geom="point", shape=21, size=3, show.legend = T)+
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.15, size=0.6, show.legend = T)+
  ...
  scale_x_continuous(name = xlab, labels=c("0", "", 0.5, 1, 1.5, 2, 3),
                     breaks = c(0, 0.3, 0.5, 1, 1.5, 2, 3), limits=c(0, 3.2))+
  scale_y_continuous(name = ylab, breaks=seq(-2, 12, 2), limits=c(-2, 13))+
  geom_text(label="Normoxia", x=2 , y=12.5, colour="black", size=5,
            show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
...
F5B<-ggplot(data=subset(delta_sat, O2=="hypoxia"), aes(x=CO2, y=rel_sat, colour=drug, fill=drug))+
  ...
  stat_summary(fun.data=mean_se, geom="errorbar", width=0.15, size=0.6, show.legend = T)+
  stat_summary(fun=mean, geom="point", shape=21, fill="white", size=3, show.legend = T)+
  geom_smooth(formula=y~x, method="loess", size=0.5, show.legend = F, alpha=0.2)+
  # stat_function(fun=normox_DMSO, colour="blue",xlim = c(0.29, 3))+
  # stat_function(fun=normox_ISO, colour="red", xlim = c(0.29, 3))+
  # stat_function(fun=normox_Am, colour="yellow", xlim = c(0.29, 3))+
  ...
  geom_text(label="Hypoxia", x=2 , y=23.5, colour="black", size=5,
              show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
