...
setwd("select path/WSB blood paramaters")
...
BOBS<-subset(BOBS, !drug=="ISO+CA") #remove fixation trial data
...
#relevel drug
BOBS$drug<-factor(BOBS$drug, levels=c("Ctrl", "ISO", "ISO+Am"),
...
Fig_Hct<-ggplot(data=mean, aes(x=drug, y=mean, fill=drug, colour=drug))+
  ...
  geom_text(label=pval_drug, x=3.5 , y=8, colour="black", size=3,
...
Fig_Hb<-ggplot(data=mean, aes(x=drug, y=mean, fill=drug, colour=drug))+
  ...
  geom_text(label=pval_drug, x=3.5 , y=0.3, colour="black", size=3,
            show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
...
Fig_MCHC<-ggplot(data=mean, aes(x=drug, y=mean, fill=drug, colour=drug))+
  ...
  geom_text(label=pval_drug, x=3.5 , y=5, colour="black", size=3,
            show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
...
Fig_pHe<-ggplot(data=mean, aes(x=drug, y=mean, fill=drug, colour=drug))+
  ...
  geom_text(label=pval_drug, x=3.5 , y=8.2, colour="black", size=3,
            show.legend = FALSE, parse=TRUE, check_overlap = TRUE, hjust=1)+
